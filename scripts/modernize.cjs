...
function walk(n,parents=[]){if(!n||typeof n!=='object')return;if(n.type==='CallExpression'&&n.callee.type==='Identifier'&&n.callee.name==='save'&&!parents.some(p=>p.type==='AwaitExpression')){edits.push([n.start,'await ']);const fn=[...parents].reverse().find(x=>/Function/.test(x.type));if(fn&&!fn.async)asyncNodes.add(fn);}
